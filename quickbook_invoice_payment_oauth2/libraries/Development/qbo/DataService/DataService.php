...
require_once(PATH_SDK_ROOT . 'Core/HttpClients/FaultHandler.php');
require_once(PATH_SDK_ROOT . 'Core/HttpClients/RestHandler.php');
require_once(PATH_SDK_ROOT . 'Core/ServiceContext.php');
require_once(PATH_SDK_ROOT . 'Core/CoreConstants.php');
require_once(PATH_SDK_ROOT . 'Core/HttpClients/SyncRestHandler.php');
require_once(PATH_SDK_ROOT . 'Core/HttpClients/RequestParameters.php');
require_once(PATH_SDK_ROOT . 'Core/Http/Serialization/JsonObjectSerializer.php');
require_once(PATH_SDK_ROOT . 'Core/Http/Serialization/SerializationFormat.php');
require_once(PATH_SDK_ROOT . 'Data/IPPAttachable.php');
require_once(PATH_SDK_ROOT . 'Data/IPPIntuitEntity.php');
require_once(PATH_SDK_ROOT . 'Data/IPPTaxService.php');
require_once(PATH_SDK_ROOT . 'Data/IPPid.php');
...
require_once(PATH_SDK_ROOT . 'Diagnostics/ContentWriter.php');
require_once(PATH_SDK_ROOT . 'XSD2PHP/src/com/mikebevz/xsd2php/Php2Xml.php');
require_once(PATH_SDK_ROOT . 'Core/OAuth/OAuth2/OAuth2LoginHelper.php');
require_once(PATH_SDK_ROOT . 'Core/OAuth/OAuth2/OAuth2AccessToken.php');
require_once(PATH_SDK_ROOT . 'Core/HttpClients/ClientFactory.php');
/**
 * Class DataServicd
 *
...
class DataService
{
    ...
    public function updateOAuth2Token($newOAuth2AccessToken)
    {
        try{
          ...
          $realmID = variable_get('quickbook_company_id','');
    ...
    public function Query($query, $startPosition = null, $maxResults = null)
    {
        ...
        // Invoice is opened by Anonymous user from the Email link to do payment
        if(empty($user->uid) || ($user->uid == 0)) {
            $current_url = strstr(request_uri(), "invoice/");
            $break_url = explode('/', $current_url);
            $company_id = $break_url[1];
            $user_entity_query = new EntityFieldQuery();
            $user_entity_query->entityCondition('entity_type', 'user')
            ->fieldCondition('field_user_company', 'value', $company_id, '=');  
            $result = $user_entity_query->execute();
            ...
            $user->uid = $company_admin_id;
        }
        $existingUser = user_load($user->uid);
                
        $user_comp = $existingUser -> field_user_company['und'][0]['value'];
        //$this->serviceContext->realmId = variable_get('quickbook_company_id');
        $this->serviceContext->realmId = $user_comp;
        ...
        if ($faultHandler) {
            ...
            try {
                ...
                $parsedResponseBody = simplexml_load_string($tmpXML);
